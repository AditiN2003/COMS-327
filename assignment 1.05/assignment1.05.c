...
void startBattle(){
  char in;
   printw("In a battle! Hit esc to leave\n");
   while((in = getch())!=27){
...
void trainterList(character_t *pc){
    int  i, j;
    int totalTrainers = 0;
    int totalCount = 0;
    char in;
    ...
    for(i = 0; i < MAP_Y; i++){
        for(j = 0; j < MAP_X; j++){
             if(world.cur_map->cmap[i][j]!=NULL && world.cur_map->cmap[i][j]->symbol != PC_SYMBOL){
                totalTrainers++;
            }
        }
    }
    character_t listTrainers[totalTrainers];
    for(i = 0; i < MAP_Y; i++){
        for(j = 0; j < MAP_X; j++){
            if(world.cur_map->cmap[i][j]){
                if(world.cur_map->cmap[i][j]->symbol != '@'){
                    listTrainers[totalCount].symbol = world.cur_map->cmap[i][j]->symbol;
                    listTrainers[totalCount].pos[dim_x] = j;
                    listTrainers[totalCount].pos[dim_y] = i;
                    totalCount++;
                }
            }
        }
    }
    printw("Number of trainers = %d", totalTrainers);
    refresh();
    
    do{
        for(i = 0; i<totalTrainers; i++){
        
         x = listTrainers[i].pos[dim_x] - pc->pos[dim_x];
         y = listTrainers[i].pos[dim_y] -  pc->pos[dim_y];
         move(terminal, 0);
        mvprintw(terminal, 0, "%c: %d %s and %d %s", listTrainers[i].symbol, abs(y), y >= 0 ? "north" : "south", abs(x), x >= 0 ? "east" : "west");
        terminal++;
        refresh();
        }
        in = getchar();
...
void enterCenterMart(){
  char in;
   while((in = getch())!='<'){
     printw("Enter '<' to exit");
   }
...
void checkBattle(pair_t des){
   if(world.cur_map->cmap[des[dim_y]][des[dim_x]] && world.cur_map->cmap[des[dim_y]][des[dim_x]]->symbol != PC_SYMBOL){
        clear();
        startBattle();
        world.cur_map->cmap[des[dim_y]][des[dim_x]]->defeated = 1;
        world.cur_map->cmap[des[dim_y]][des[dim_x]] = NULL;
   }
}

void movePC(character_t *pc, char *str){
  pair_t des;
    des[dim_x] = pc->pos[dim_x];
    des[dim_y] = pc->pos[dim_y];

    if(strcmp(str,"UL")==0){
            if( des[dim_x]- 1 != 0 &&  des[dim_x]- 1 != MAP_X - 1&& des[dim_y] - 1 != 0 && des[dim_y]- 1 != MAP_Y - 1){

              if(world.cur_map->map[des[dim_y]-1][ des[dim_x]-1]==ter_path||world.cur_map->map[pc->pos[dim_y]-1][pc->pos[dim_x]-1]==ter_center||world.cur_map->map[pc->pos[dim_y]-1][pc->pos[dim_x]-1]==ter_mart||world.cur_map->map[pc->pos[dim_y]][pc->pos[dim_x]]==ter_clearing||world.cur_map->map[pc->pos[dim_y]][pc->pos[dim_x]]==ter_grass){
                des[dim_y] --;
                des[dim_x] --;
                checkBattle(des);
               }
      ...
      printw("\n %d", des[dim_y]);
         if( des[dim_y] - 1 != 0 && des[dim_y] - 1 != MAP_Y - 1){

          if(world.cur_map->map[des[dim_y]-1][ des[dim_x]]==ter_path||world.cur_map->map[pc->pos[dim_y]-1][pc->pos[dim_x]]==ter_center||world.cur_map->map[pc->pos[dim_y]-1][pc->pos[dim_x]]==ter_mart||world.cur_map->map[pc->pos[dim_y]-1][pc->pos[dim_x]]==ter_clearing||world.cur_map->map[pc->pos[dim_y]-1][pc->pos[dim_x]]==ter_grass){
            des[dim_y]--;
            checkBattle(des);
           }
         ...
         if(des[dim_y] - 1 != 0 && des[dim_y] - 1 != MAP_Y - 1 &&  des[dim_x]+1 != 0 &&  des[dim_x] + 1 != MAP_X - 1){

          if(world.cur_map->map[des[dim_y]-1][ des[dim_x]+1]==ter_path||world.cur_map->map[pc->pos[dim_y]-1][pc->pos[dim_x]+1]==ter_center||world.cur_map->map[pc->pos[dim_y]-1][pc->pos[dim_x]+1]==ter_mart||world.cur_map->map[pc->pos[dim_y]-1][pc->pos[dim_x]+1]==ter_clearing||world.cur_map->map[pc->pos[dim_y]-1][pc->pos[dim_x]+1]==ter_grass){
             des[dim_y]--;
              des[dim_x]++;
              checkBattle(des);
          }
         ...
         if( des[dim_x]+1 != 0 &&  des[dim_x] + 1 != MAP_X - 1){

          if(world.cur_map->map[des[dim_y]][ des[dim_x]+1]==ter_path||world.cur_map->map[pc->pos[dim_y]][pc->pos[dim_x]+1]==ter_mart||world.cur_map->map[pc->pos[dim_y]][pc->pos[dim_x]+1]==ter_center||world.cur_map->map[pc->pos[dim_y]][pc->pos[dim_x]+1]==ter_clearing||world.cur_map->map[pc->pos[dim_y]][pc->pos[dim_x]+1]==ter_grass){
             des[dim_x]++;
             checkBattle(des);
          }
         ...
         if(des[dim_y] + 1 != 0 && des[dim_y] + 1 != MAP_Y - 1 &&  des[dim_x]+1 != 0 &&  des[dim_x]+ 1 != MAP_X - 1){
         if(world.cur_map->map[des[dim_y]+1][ des[dim_x]+1]==ter_path||world.cur_map->map[pc->pos[dim_y]+1][pc->pos[dim_x]+1]==ter_mart||world.cur_map->map[pc->pos[dim_y]+1][pc->pos[dim_x]+1]==ter_center||world.cur_map->map[pc->pos[dim_y]+1][pc->pos[dim_x]+1]==ter_clearing||world.cur_map->map[pc->pos[dim_y]+1][pc->pos[dim_x]+1]==ter_grass){
           des[dim_y]++;
             des[dim_x]++;
             checkBattle(des);
         ...
         if( des[dim_y] + 1 != 0 && des[dim_y] + 1 != MAP_Y - 1){
          if(world.cur_map->map[des[dim_y]+1][ des[dim_x]]==ter_path||world.cur_map->map[pc->pos[dim_y]+1][pc->pos[dim_x]]==ter_mart||world.cur_map->map[pc->pos[dim_y]+1][pc->pos[dim_x]]==ter_center||world.cur_map->map[pc->pos[dim_y]+1][pc->pos[dim_x]]==ter_clearing||world.cur_map->map[pc->pos[dim_y]+1][pc->pos[dim_x]]==ter_grass){
             des[dim_y]++;
             checkBattle(des);
          }
        }
        ...
        if(world.cur_map->map[pc->pos[dim_y]][pc->pos[dim_x]]==ter_mart||world.cur_map->map[pc->pos[dim_y]][pc->pos[dim_x]]==ter_center){
          clear();
           printw("Enter '<' to exit");
           enterCenterMart();
        }
    ...
    world.cur_map->cmap[des[dim_y]][des[dim_x]] = pc;
    
    if(strcmp(str,"5")==0){
        pc->next_turn =  pc->next_turn +move_cost[char_pc][world.cur_map->map[des[dim_y]][des[dim_x]]];
    } else if((des[dim_x] != pc->pos[dim_x] || des[dim_y] != pc->pos[dim_y])){
        pc->next_turn =  pc->next_turn +move_cost[char_pc][world.cur_map->map[des[dim_y]][des[dim_x]]];
    }
    

    pc->pos[dim_x] = des[dim_x];
    pc->pos[dim_y] = des[dim_y];
...
{
  int x, y;
heap_delete(&world.cur_map->turn); //nothing
